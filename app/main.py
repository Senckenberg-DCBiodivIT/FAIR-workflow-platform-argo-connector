...
def process_workflow(name: str, namespace: str, skip_content: bool):
    ...
    wfl = argo.get_workflow_information(
        settings.argo_base_url, settings.argo_token, namespace, name, verify_cert=False
    )
...
def healthcheck():
    ...
    """
    cordra_health = cordra.check_health(
        settings.cordra_base_url, settings.cordra_user, settings.cordra_password
    )
    argo_health = argo.check_health(
        settings.argo_base_url,
        settings.argo_token,
        settings.argo_default_namespace,
        verify_cert=False,
    )
    response = {
    ...
    if not cordra_health or not argo_health:
...
def notify(
    ...
    skip_content: bool = Query(
        False,
        description="If set, does not download artifacts, but insert empty files for debugging purposes",
    ),
):
    """
    ...
    if wfl["status"]["phase"] != "Succeeded":
        if len(unsucceeded_nodes) > 0:
            ...
            logger.info(
                "Workflow still running, but only on exit handler. Continue processing"
            )
    ...
    return JSONResponse(
        ...
        content={
            ...
            "workflow_namespace": wfl["metadata"]["namespace"],
            "artifacts": [
                {"node_id": node_id, "path": path} for (node_id, _, path) in artifacts
            ],
            "skip_content": skip_content,
        },
    )
...
@app.get(
    ...
    dependencies=[Depends(check_auth)],
    response_model=WorkflowListResponseModel,
)
def workflow_detail(
...
@app.get(
    "/workflow/list",
    dependencies=[Depends(check_auth)],
    response_model=List[WorkflowListResponseModel],
)
def list():
    """
    Lists workflows
...
async def check_workflow(
    ...
    try:
        checked_workflow = argo.verify(
            settings.argo_base_url,
            settings.argo_token,
            content,
            namespace=content.get("metadata", {}).get(
                "namespace", settings.argo_default_namespace
            ),
            verify_cert=False,
        )
        workflow_parameters = [
            {"name": param["name"], "value": param["value"]}
            for param in checked_workflow.get("spec", {})
            .get("arguments", {})
            .get("parameters", [])
        ]
...
async def submit(
    ...
    submitterName: str = Form(
        ..., description="Name of the user submitting the workflow"
    ),
    submitterId: str = Form(
        ...,
        description="Id of the user submitting the workflow, should be ORCID, ROR or doi url",
        examples=["https://orcid.org/0000-1234-4567-8910"],
    ),
    ...
    keywords: str = Form(
        None,
        description="Keywords of the workflow",
        examples=["keyword1,keyword2,keyword3"],
    ),
    dryRun: bool = Form(
        False,
        description="Whether to perform a dry run of the workflow or actually submit it",
    ),
    webhookURL: str = Form(
        None,
        description="If provided, webhook will be triggered once the workflow is complete.",
    ),
    force: bool = Form(
        True,
        description="Whether to force repeated execution if workflow signature already exists.",
    ),
):
    """
    ...
    if keywords is not None:
        content["metadata"]["annotations"]["argo-connector/keywords"] = ",".join(
            [x.strip() for x in keywords.split(",")]
        )
    if title is not None:
        content["metadata"]["annotations"]["workflows.argoproj.io/title"] = title
    if description is not None:
        content["metadata"]["annotations"]["workflows.argoproj.io/description"] = (
            description
        )
    if webhookURL is not None:
        content["metadata"]["annotations"]["argo-connector/webhookURL"] = str(
            webhookURL
        )
    ...
    checked_workflow["metadata"]["labels"]["workflows.argoproj.io/signature"] = (
        workflow_signature
    )
    workflow_parameters = [
        {"name": param["name"], "value": param["value"]}
    ...
    try:
        ...
        wfl_metadata = argo.submit(
            settings.argo_base_url,
            settings.argo_token,
            deepcopy(checked_workflow),
            namespace=namespace,
            dry_run=dryRun,
            verify_cert=False,
        )
