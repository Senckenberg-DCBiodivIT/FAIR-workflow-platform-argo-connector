...
class NotificationResponseModel(BaseModel):
    ...
    skip_content: bool = Field(
        False,
        description="Whether to perform a dry run of the object inserting or actually submit data to cordra",
    )
    workflow_status: str = Field(
        "Succeeded", description="Current state of the workflow"
    )
...
class WorkflowResponseModel(BaseModel):
    ...
    workflow_id: str = Field(None, description="Workflow identifier")
...
class WorkflowListResponseModel(BaseModel):
    ...
    submitterName: str | None = Field(
        None, description="Name of the user submitting the workflow"
    )
