...
def check_health(
    host: str, token: str, namespace: str, verify_cert: bool = True
) -> bool | str:
...
def parse_artifact_list(wfl: dict[str:Any]) -> list[tuple[str, str, str]]:
    ...
    for node_name in wfl["status"]["nodes"]:
        ...
        if "outputs" not in node or "artifacts" not in node["outputs"]:
            continue
        ...
        for artifact in artifacts:
            ...
            if "deleted" in artifact and artifact["deleted"]:
                continue
            if (
                "artifactGC" in artifact
                and artifact["artifactGC"]["strategy"] != "Never"
            ):
                continue
...
def _recursive_artifact_reader(
    url: str,
    argo_token: str,
    path: str,
    ...
    if is_download:
        ...
        for link in soup.find_all("a"):
            ...
            yield from _recursive_artifact_reader(
                new_url, argo_token, new_path, verify_cert, chunk_size
            )
...
def artifact_reader(
    ...
    namespace: str,
    workflow_name: str,
    artifact_list: list[tuple[str, str, str]],
    verify_cert: bool = True,
):
...
def verify(
    ...
    wfl = workflow_service_api.IoArgoprojWorkflowV1alpha1Workflow(
        metadata=workflow.get("metadata", {}),
        spec=workflow.get("spec", {}),
        kind=workflow.get("kind", "Workflow"),
        _configuration=argo_workflows.configuration.Configuration(),
        _check_type=False,
    )
    model = workflow_service_api.IoArgoprojWorkflowV1alpha1WorkflowLintRequest(
        namespace=namespace, workflow=wfl
    )
...
def submit(
    ...
    if "generateName" not in workflow.get("metadata", {}):
    ...
    model = workflow_service_api.IoArgoprojWorkflowV1alpha1WorkflowCreateRequest(
        namespace=namespace, workflow=wfl, kind="Workflow", server_dry_run=dry_run
    )
...
def get_workflow_by_signature(
    ...
    r = requests.get(
        url=f"{url}/api/v1/workflows/{namespace}", params=params, headers=headers
    )
